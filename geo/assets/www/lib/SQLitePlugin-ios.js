...
(function () {
    ...
    SQLitePlugin = function (dbargs, openSuccess, openError) {
        ...
        this.dbargs = dbargs;
        /*
        Original - in Android, the sqlite plugin added ".db" to the file name in the
        java class, for iOS it does it in javascript - either way, it messes up
        sqlite dbs that don't end in ".db", like mbtiles files
        ...
        */
        ...
        this.openSuccess || (this.openSuccess = function () {
            console.log("DB opened: " + that.dbname);
        });
    ...
    SQLiteGetDbPath = function () {
        if (arguments.length != 2) return null;
        console.log("SQLitePlugin::getDatabasePath");
        cordova.exec(arguments[1], function (error) {
            console.log("Error occurred while attempting to derive db path: " + error);
        }, "SQLitePlugin", "getDatabasePath", [arguments[0]]);
    };
    SQLiteDbExists = function () {
        console.log("SQLitePlugin, argument length: " + arguments.length);
        if (arguments.length != 2) return null;
        console.log("SQLitePlugin::databaseExists");
        cordova.exec(arguments[1], function (error) {
            console.log("Error occurred while attempting to derive db path: " + error);
        }, "SQLitePlugin", "databaseExists", [arguments[0]]);
    };
    ...
    root.sqlitePlugin = {
         ...
         * Start custom
         *************************************************************************/
        getDatabasePath: SQLiteGetDbPath,
        databaseExists: SQLiteDbExists
        /*************************************************************************
         * End custom
         *************************************************************************/
    };
