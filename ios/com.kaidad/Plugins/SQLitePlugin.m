...
-(void) getDatabasePath:(CDVInvokedUrlCommand*)command {
    CDVPluginResult* pluginResult = nil;
    
    //for iOS, we don't care about the db name, as is the case for android - all DBs just go
    //to the appDocsPath
    pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:appDocsPath];
    
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}

-(void) databaseExists:(CDVInvokedUrlCommand *)command {
    CDVPluginResult* pluginResult = nil;
    NSString *dbname = [self getDBPath:[command.arguments objectAtIndex:0]];
    if (dbname != nil && [dbname length] > 0) {
        BOOL fileExists = [[NSFileManager defaultManager] fileExistsAtPath:dbname];
        NSLog(@"Checking if file exists: %@, %s", dbname, (fileExists ? "exists" : "does NOT exist"));
        
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsBool:fileExists];
